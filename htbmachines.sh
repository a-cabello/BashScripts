...
function helpPanel(){
  ...
  echo -e "\t${purpleColour}y)${endColour} ${grayColour}Obtener link tutorial de maquina en Youtube${endColour}"
...
function updateFiles(){
  ...
  if [ ! -f bundle.js ]; then
    ...
    #Volvemos a acticar la vista del cursor
    tput cnorm
  else
    tput civis
...
function searchMachine(){
  ...
  if [ "$machineName_checker" ]; then 
    ...
    echo -e "\n${redColour}[!] La maquina solicitada no existe${endColour}\n"
  fi
}
...
function searchIP(){
  ...
  if [ "$machineName" ]; then
    ...
    echo -e "\n${redColour}[!] La direccion IP solicitada no existe${endColour}\n"
  fi
...
function getYoutubeLink(){
  machineName="$1"
  youtubeLink="$(cat bundle.js | awk "/name: \"Forge\"/,/resuelta:/" | grep -vE "id:|sku:|resuelta:" | tr -d '""' | tr -d ',' | sed 's/^ *//' | grep "youtube" | awk 'NF{print $NF}')"

  if [ $youtubeLink ]; then
    echo -e "\n${yellowColour}[+]${endColour} ${grayColour}El tutorial para${endColour} ${blueColour}$machineName${endColour} ${grayColour}esta en:${endColour} ${purpleColour}$youtubeLink${endColour}\n"
  else
    echo -e "\n${redColour}[!] El link tutorial solicitado no se encuentra disponible${endColour}\n"
  fi
}

function getMachinesDifficulty(){
  dificulty=$1
  results_check="$(cat bundle.js | grep "dificultad: \"$dificulty\"" -B 5 | grep "name:" | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column)"
  if [ "$results_check" ]; then
    echo -e "\n ${yellowColour}[+]${endColour} ${grayColour}Representando las maquinas que tienen un nivel de dificultad${endColour} ${blueColour}$dificulty${endColour}${grayColour}:${endColour}\n"
   cat bundle.js | grep "dificultad: \"$dificulty\"" -B 5 | grep "name:" | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column 
    
  else
    echo -e "\n${redColour}[!] La dificultad solicitada no existe${endColour}\n"
  fi
}
...
function getOSMachines(){
  ...
  if [ "$os_results" ]; then
    ...
    echo -e "\n${redColour}[!] El sistema operativo solicitado no existe${endColour}\n"
  fi
...
function getOSDifficultyMachines(){
  ...
  if [ "$check_results" ]; then
    echo -e "\n${yellowColour}[+]${endColour} ${grayColour}Listando maquinas de dificultad${endColour} ${blueColour}$difficulty${endColour} ${grayColour}que tengan como sistema operativo${endColour} ${purpleColour}$os${endColour}${grayColour}:${endColour}\n"
...
function getSkill(){
  ...
  if [ "$check_skill" ]; then
    echo -e "\n${yellowColour}[+]${endColour} ${grayColour}A continuacion se representan las maquinas con la skill solicitada de${endColour} ${blueColour}$skill${endColour}${grayColour}:${endColour}\n"
    cat bundle.js | grep "skills:" -B 6 | grep "$skill" -i -B 6 | grep "name:" | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column
  else
    echo -e "\n${redColour}[!] La skill solicitada no se ha encontrado${endColour}\n"
  fi
}
...
#Nos permite poder meter argumentos
